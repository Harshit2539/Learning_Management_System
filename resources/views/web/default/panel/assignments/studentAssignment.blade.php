    ...
    <div class="page-header-wrapper anim-fade-up">
        <div class="page-header-inner">
            <div class="page-header-left">
                ...
                <div>
                    <h1 class="page-header-title">Student's Assignment Preview</h1>
        ...
        <div class="table-responsive">
            <table id="assignmentTable" class="display" style="width:100%">
                ...
                <tbody>
                    ...
                    @php
                        $initials = substr($assignment->student_name ?? 'U', 0, 1);
                    ...
                    @endphp
                    <tr
                        data-name="{{ strtolower($assignment->student_name ?? '') }}"
                    ...
                    >
                        ...
                        <td>
                            <div class="user-name-cell">
                                ...
                                <span class="user-name-text">{{ $assignment->student_name ?? 'N/A' }}</span>
                            </div>
                        </td>
                        ...
                        <td>
                            @if($hasSubmitted)
                                <form action="{{ route('panel.assignments.submitMarks') }}" method="POST" class="marks-form" data-max="{{ $assignment->total_marks }}">
                                    @csrf
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success === true || response.status === 'success') {
                    ...
                    // Optional: Update any displayed marks in the UI
                    var marksCell = $(form).closest('tr').find('.marks-display');
                    if (marksCell.length && response.data && response.data.marks) {
                        // Update if you have a separate display
                    }
                } else {
            ...
            error: function(xhr, status, error) {
                ...
                var errorMsg = 'Failed to save marks. Please try again.';
                
                // Try to parse error response
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    errorMsg = xhr.responseJSON.message;
                } else if (xhr.responseText) {
                    // Check if response contains HTML (redirect)
                    if (xhr.responseText.indexOf('<!DOCTYPE') !== -1 || xhr.responseText.indexOf('<html') !== -1) {
                        errorMsg = 'Session expired or page reloaded. Please refresh and try again.';
                    }
                }
                
                showToast(errorMsg, 'error');
                
                // Optional: Reload page on authentication error
            ...
            complete: function() {
                btn.removeClass('loading').html(originalSVG);
            }
        });
    });
